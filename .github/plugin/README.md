...
The plugin is organized around four stages:

1. `specrail-init`: Bootstrap a new `.specrail/` project when the repository is not initialized.
2. `specrail-workflow`: Gather and structure features and outcomes through follow-up questions.
3. `specrail-testing`: Plan, register, generate, and update tests for each outcome.
4. `specrail-activation`: Execute the workflow in order with `implement`, `verify`, and `advance`.
...
If the project is already partway through the workflow, start from the stage that matches the current state instead of starting over.
