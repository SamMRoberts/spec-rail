...
handoffs:
  - label: Return To Automatic Workflow
    ...
    prompt: Setup is complete or the repository was already initialized. Re-check `specrail_status` and continue the automatic SpecRail workflow from live state.
    send: true
---
...
- If the project is already initialized, summarize the current state and **use the `Return To Automatic Workflow` handoff**.
...
- Once initialization is confirmed complete, **immediately use the `Return To Automatic Workflow` handoff** without pausing.
...
- Do not list "Natural next steps" or stop with recommendations. Always execute the `Return To Automatic Workflow` handoff once setup is complete.
...
- Prefer `vscode_askQuestions` when it is available for setup choices.
...
- Before handing off, confirm that setup is complete and ready for the automatic coordinator to continue.
