...
pub fn remove_required_test(
    ...
    let conn = open_db(dir);
    conn.execute(
        "DELETE FROM tests WHERE feature_id = ?1 AND outcome_id = ?2 AND path = ?3",
        params![feature_id, outcome_id, test_file],
    )
    .unwrap();
}
...
pub fn outcome_required_tests(dir: &TempDir, feature_id: &str, outcome_id: &str) -> Vec<String> {
    ...
    let json: String = conn
        .query_row(
            "SELECT required_tests FROM outcomes WHERE feature_id = ?1 AND id = ?2",
